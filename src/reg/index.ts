...
export const checkStrByReg = function (str: string, type: string): boolean {
  return (
    {
      // 验证不能包含字母
      isNoWord: /^[^A-Za-z]*$/g.test(str),
      // 验证中文和数字
      isCHNAndEN:
      ...
      isWeChatNum: /^[a-zA-Z][-_a-zA-Z0-9]{5,19}$/g.test(str),
      // 验证16进制颜色
      isColor16: /^#?([a-fA-F0-9]{6}|[a-fA-F0-9]{3})$/g.test(str),
      // 验证火车车次
      isTrainNum: /^[GCDZTSPKXLY1-9]\d{1,4}$/g.test(str),
      // 验证必须带端口号的网址(或ip)
      isHttpAndPort: /^((ht|f)tps?:\/\/)?[\w-]+(\.[\w-]+)+:\d{1,5}\/?$/g.test(
        str,
      ),
      // 验证网址(支持端口和"?+参数"和"#+参数)
      isRightWebsite:
      ...
      // 验证统一社会信用代码
      isCreditCode:
        /^[0-9A-HJ-NPQRTUWXY]{2}\d{6}[0-9A-HJ-NPQRTUWXY]{10}$/g.test(str),
      // 验证版本号格式必须为X.Y.Z
      isVersion: /^\d+(?:\.\d+){2}$/g.test(str),
      // 验证图片链接地址（图片格式可按需增删）
      isImageUrl:
      ...
      // 验证中文姓名
      isChineseName: /^(?:[\u4e00-\u9fa5·]{2,16})$/g.test(str),
      // 验证英文姓名
      isEnglishName: /(^[a-zA-Z]{1}[a-zA-Z\s]{0,20}[a-zA-Z]{1}$)/g.test(str),
      // 验证车牌号(新能源)
      ...
      isMPRelaxed: /^(?:(?:\+|00)86)?1[3-9]\d{9}$/g.test(str),
      ...
      isLandlineTelephone: /\d{3}-\d{8}|\d{4}-\d{7}/g.test(str),
      // 验证护照（包含香港、澳门）
      isPassport:
      ...
      // 验证小数
      isDecimal: /^\d+\.\d+$/g.test(str),
      // 是否为整数
      isInteger: /^d+$/.test(str),
      // 验证数字字符串
      isNumberStr: /^\d{1,}$/g.test(str),
      // 验证qq号格式
      isQQNum: /^[1-9][0-9]{4,10}$/g.test(str),
      // 验证数字和字母组成
      isNumAndStr: /^[A-Za-z0-9]+$/g.test(str),
      // 验证英文字母
      isEnglish: /^[a-zA-Z]+$/g.test(str),
      // 验证大写英文字母
      isCapital: /^[A-Z]+$/g.test(str),
      // 验证小写英文字母
      isLowercase: /^[a-z]+$/g.test(str),
    }[type] || false
  )
}
